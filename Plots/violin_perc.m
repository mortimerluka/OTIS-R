...
%}
%%
function[h,L,MX,MED,bw]=violin_perc(Y,varargin)
...
i=1;
U_perc = NaN(100,size(Y,2));
F_perc = NaN(100,size(Y,2));
for i=1:size(Y,2)
    ...
    perc(i,:) = prctile(Y{i},[25,75]);
    ind = find(u>=perc(i,1) & u<=perc(i,2));
    U_perc(1:length(ind),i) = U(ind,i);
    F_perc(1:length(ind),i) = F(ind,i);
...
for i=i:size(Y,2)
    ...
    hold on
    perp(i)=fill([rmmissing(F_perc(:,i)+i);rmmissing(flipud(i-F_perc(:,i)))],[rmmissing(U_perc(:,i));flipud(rmmissing(U_perc(:,i)))],fc(i,:),'FaceAlpha',0.4,'EdgeColor','none');
    if setX == 0
        if plotmean == 1
