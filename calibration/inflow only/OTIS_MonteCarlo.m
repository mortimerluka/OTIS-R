...
% Preallocate the objective function for the computed BTC    
    ...
    Data.nRMSE_N_Qfix = zeros(n,1);
    Data.nRMSE_R290_Qfix = zeros(n,1);
    Data.nRMSE_R206_Qfix = zeros(n,1);
...
for i = 1:n             % i defines every run
    ...
    %% Run model with sampled discharge
    ...
    %% run model with fixed discharge

    %%%%%%%%%%% NaCl %%%%%%%%%%%%%%%%%%%%

    % Run the forward model for parameter set i for NaCl
        Model_N = OTIS_run_N_Qfix(Instate,i,OTIS_hypercube_input,OSFLAG);
    ...
    % Run the ObjFun function for NaCl
        Data.nRMSE_N_Qfix(i) = ObjFun_N(NaCl_down,BTC_temp_N);
    ...
    Instate.k2=OOO.k2_290;
    % Run the forward model for parameter set i for NaCl
        Model_R = OTIS_run_R_Qfix(Instate,i,OTIS_hypercube_input,OSFLAG);
    ...
    % Run ObjFun function for Rn
        Data.nRMSE_R290_Qfix(i) = ObjFun_R(Rn_down,Sim_R);
    ...
    % Run ObjFun function for Rn
        Data.nRMSE_R206_Qfix(i) = ObjFun_R(Rn_down,Sim_R);
