function Model_R=OTIS_run_R(Instate,i,OTIS_hypercube_input,OSFLAG)

% Modified from OTIS-MCAT (Ward et al., 2017)

PRTOPT  = Instate.PRTOPT;                          
PSTEP   = Instate.PSTEP(2);                
TSTEP   = Instate.TSTEP(2);                 
...
IDECAY  = Instate.IDECAY(2);
ISORB   = Instate.ISORB;
IGASEX  = Instate.IGASEX(2);

LAMBDA  = Instate.LAMBDA(2);
LAMBDA2 = Instate.LAMBDA2(2);
PRODUCTION = Instate.PRODUCTION(2);
 
NPRINT  = Instate.NPRINT;
IOPT    = Instate.IOPT;
PRINTLOC= Instate.PRINTLOC;
 
NBOUND  = Instate.NBOUND_R;
IBOUND  = Instate.IBOUND_R;
USTIME_USBC = Instate.USTIME_USBC_R;
 
QSTEP   = Instate.QSTEP;

Q_up  = OTIS_hypercube_input(i,4);
Q_down = OTIS_hypercube_input(i,5);

QSTART = Q_up;

q_I = (Q_down-Q_up)/PRINTLOC;
q_I(q_I<1e-16)=0;
q_O=0;
...
DEPTH = Instate.DEPTH;
CLATIN = Instate.CLATIN(2);

k2 = Instate.k2; % gas exchange coefficient
GASEX = k2*DEPTH; % gas exchange velocity

% Modify the PARAMS:INP file -> This comes directly from OTIS_Mcat
% Modified to work for OTISR

template=fopen('params_R.template', 'rt');
...
template=fopen('q_R.template','rt');
...
%     ctest = isstrprop(c{pp,1}, 'alpha');
    ...
    if sum(atest) > 0.0
        xdistance(1,pp) = sscanf(a{pp,1}, '%13E');  
    else
        xdistance(1,pp) = sscanf('0','%13E');
    end
...
Model_R.xdistance=xdistance;
Model_R.conc_Channel=conc_Channel;
