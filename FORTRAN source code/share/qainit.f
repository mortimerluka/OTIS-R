...
*
************************************************************************
      SUBROUTINE QAINIT(IMAX,Q,AREA,DEPTH,QVALUE,AVALUE,DVALUE,QWT,
     &                  QINDEX,QINVAL,CLVAL,QLATIN,CLATIN,DSDIST,USDIST,
     &                  NSOLUTE)
...
*
      ...
      DOUBLE PRECISION Q(*),AREA(*),DEPTH(*),QVALUE(*),AVALUE(*),
     &                 DVALUE(*),QWT(*),QINVAL(*),QLATIN(*),DSDIST(*),
     &                 USDIST(*),CLATIN(MAXSEG,*),CLVAL(MAXFLOWLOC+1,*)
*
*     function declaration
*
      DOUBLE PRECISION WTAVG
*
*     initialize flow rates and areas and depths
*
       DO 10 I = 1,IMAX
         J = QINDEX(I)
         Q(I) = QVALUE(J-1) + QWT(I) * (QVALUE(J) - QVALUE(J-1))
         AREA(I) = AVALUE(J-1) + QWT(I) * (AVALUE(J) - AVALUE(J-1))
         DEPTH(I) = DVALUE(J-1) + QWT(I) * (DVALUE(J) - DVALUE(J-1))
 10   CONTINUE
*
